...
def run_scenario(inputs: ScenarioInputs, data: dict) -> ScenarioResult:
    ...
    for r in rows:
        ...
        r["closure_cost"] = (finance.closure_cost(r["residual_emis"], inputs.carbon_price,
                                                  a["neutralization_share"])
                             if y == NETZERO_YEAR else 0.0)
    ...
    outcome_2030 = dict(
        reduction=y30["reduction_vs_2019"], intensity=y30["intensity"],
        ...
        **targets.classify_2030(y30["reduction_vs_2019"], tuple(data["config"]["meta"]["target_band"])))
    ...
    return ScenarioResult(pathway, outcome_2030, physical_2040, financial_2040, diagnostics)
